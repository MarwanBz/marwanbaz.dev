...
export const marwanData: UnifiedData = {
  ...
  projects: [
    ...
    },
    {
      ...
      isFeatured: true,
      summary:
        "B2B supply chain marketplace built for Saudi businesses, with trilingual support (Arabic, English, and Urdu), a full store app, and an internal admin dashboard.",
      imageUrl: "/assets/tawreedplus.png",
      coreFunctionalities: [
        "Trilingual (Arabic/English/Urdu) product catalog with RTL/LTR layout support",
        ...
        "Dashboard: Analytics reports — Sales, Purchase, Profit, Customer Pills, and Sales Analysis pages",
        "Dashboard: Coupon management — TypeScript types, Zod validation, and server actions",
        "Dashboard: Orders invoice printing flow",
      ],
      role: "Frontend Developer (Store App & Dashboard)",
      technologies: [
        "Next.js 15",
        "TypeScript",
        "Tailwind CSS v4",
      ...
      expectedOutcome:
        "A fast, trilingual B2B platform that actually works — a clean purchasing experience for buyers, and a dashboard that gives the business visibility into sales, orders, and coupons.",
      initialDesigns: ["/assets/tawreedplus.png"],
      spotlightFeature: {
        title: "Frontend Across Store + Dashboard",
        description:
          "Built the full frontend for both the customer-facing store and the admin dashboard — from product catalog and checkout to sales reports, coupon management, and invoice printing — all on Next.js 15 App Router.",
      },
      technicalChallenges: [
        "Getting RTL/LTR to work seamlessly across three languages (Arabic, English, Urdu) with next-intl",
        "Managing server state with TanStack Query across a large product catalog",
        "Building type-safe forms with React Hook Form and Zod validation",
        "Integrating Google Maps API with locale-aware rendering for all three languages",
        "Learning and working with Server Actions on the dashboard — new pattern for me",
        "Migrating to Tailwind CSS v4's new config and utility patterns",
      ],
      solutions: [
        "Used next-intl for i18n routing, locale-aware layouts, and RTL/LTR direction switching",
        "Leveraged TanStack Query for smart caching and background refetching",
        "Built reusable, type-safe form components with React Hook Form + Zod",
        "Integrated Google Maps JavaScript API with locale switching for delivery address selection",
        "Used Server Actions on the dashboard for coupon management and invoice flows",
        "Applied Framer Motion for smooth page transitions and animations",
      ],
      currentStatus: {
        users: "Live — deployed and serving customers in Saudi Arabia",
        feedback: "Positive reception for clean UI and multi-language support",
      },
      lessonsLearned: [
        "How Next.js 15 App Router and server components actually work — used across both apps",
        "Trilingual i18n with next-intl — Arabic, English, and Urdu with proper RTL/LTR handling",
        "Server Actions — got real hands-on experience building the dashboard with them",
        "TanStack Query v5 for complex caching, background refetching, and mutations",
        "Shadcn/ui + Radix UI primitives — how accessible component systems are actually built",
        "Tailwind CSS v4 migration and the new utility and config patterns",
      ],
      frameworkExperience:
        "Both the store app and the dashboard ran on Next.js 15 App Router. The dashboard leaned heavily on Server Actions — that's where I got real hands-on experience with them. TanStack Query handled all the client-side state and caching on the store side.",
      accessibilityLearnings:
        "We used shadcn/ui, which is built on Radix UI primitives — so accessibility was baked in from the start. Radix handles focus management, ARIA attributes, and keyboard navigation out of the box. The bigger custom challenge was the trilingual RTL/LTR layout support across Arabic, English, and Urdu.",
      impact:
        "Delivered a live B2B marketplace for Saudi businesses — built the store app end to end (auth, catalog, cart, checkout, invoices) and contributed to the admin dashboard with analytics reports, coupon management, and invoice printing. Three languages: Arabic, English, and Urdu. It's live.",
      screenshots: [
        {
